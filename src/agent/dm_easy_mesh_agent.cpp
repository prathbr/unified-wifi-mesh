...
int dm_easy_mesh_agent_t::analyze_channel_sel_req(em_bus_event_t *evt, wifi_bus_desc_t *desc,bus_handle_t *bus_hdl)
{
    ...
    unsigned int highest_anticipated_preference = 0;
    ...
    for (i = 0;i < channel_sel->num; i++) {
        ...
        for (unsigned int ch_idx = 0; ch_idx < dm_op_class->num_channels; ch_idx++) {
            if (dm_op_class->channel_pref[ch_idx] > highest_anticipated_preference) {
